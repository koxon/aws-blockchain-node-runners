...
describe("NearHANodesStack", () => {
  test("synthesizes the way we expect", () => {
    const app = new cdk.App();

    // Create the NearHANodesStack.
    const nearHANodesStack = new NearHANodesStack(app, "near-sync-node", {
    ...
    const template = Template.fromStack(nearHANodesStack);
    ...
    template.hasResourceProperties("AWS::AutoScaling::AutoScalingGroup", {
      AutoScalingGroupName: `near-ha-nodes-${config.baseNodeConfig.nodeConfiguration}`,
    ...
    template.hasResourceProperties("AWS::AutoScaling::LifecycleHook", {
      ...
      LifecycleHookName: `near-ha-nodes-${config.baseNodeConfig.nodeConfiguration}`,
    ...
    template.hasResourceProperties("AWS::ElasticLoadBalancingV2::LoadBalancer", {
      LoadBalancerAttributes: [
        ...
        },
        {
         ...
         Value: `near-ha-nodes-${config.baseNodeConfig.nodeConfiguration}`
        }
       ],
       Scheme: "internal",
       SecurityGroups: [
