...
if [ -n "$DATA_VOLUME_ID" ]; then
  ...
  if [ $(df --output=target | grep -c "/var/near/data") -lt 1 ]; then
    ...
    if [ $(grep -c "data" /etc/fstab) -gt 0 ]; then
      SED_REPLACEMENT_STRING="$(grep -n "/var/near/data" /etc/fstab | cut -d: -f1)s#.*#$DATA_VOLUME_FSTAB_CONF#"
...
if [ -n "$ACCOUNTS_VOLUME_ID" ]; then
  ...
  if [ $(df --output=target | grep -c "/var/near/accounts") -lt 1 ]; then
    ...
    if [ $(grep -c "/var/near/accounts" /etc/fstab) -gt 0 ]; then
      SED_REPLACEMENT_STRING="$(grep -n "/var/near/accounts" /etc/fstab | cut -d: -f1)s#.*#$ACCOUNTS_VOLUME_FSTAB_CONF#"
      sudo cp /etc/fstab /etc/fstab.bak
      sudo sed -i "$SED_REPLACEMENT_STRING" /etc/fstab
    else
